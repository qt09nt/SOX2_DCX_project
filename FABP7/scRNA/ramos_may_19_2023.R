...
for(sample.curr in samples){
  ...
  setwd(paste0("D:/ramos/", sample.curr))
    ...
    # AAACCCAAGAACTGAT-1      ramos        715          575   2.937063
    # AAACCCAAGAAGTCAT-1      ramos        832          642   3.365385
    # AAACCCAAGACTACCT-1      ramos        664          468  15.662651
    # AAACCCAAGAGAGCAA-1      ramos        606          471   5.940594
    # AAACCCAAGAGAGTGA-1      ramos       2925         1753   4.581197
    #
  ...
  ######### Normalizing the Data  
    ...
    #Scale data 
    #May 19, 2023 12PM - keep all features for now and don't 
    #regress out mitochondrial related genes
    
    #optional: regress out mitochondrial contamination which
    #may give unwanted sources of variation
    all.genes <- rownames(pbmc)
    pbmc <- ScaleData(pbmc, features = all.genes)
    
    #OPTIONAL: remove sources of variation due to mitchondrial genes
    ...
    # 
    # We assign scores in the CellCycleScoring() function, which stores S and G2/M scores 
    # in object meta data, along with the predicted classification of each cell in either G2M, 
    # S or G1 phase. CellCycleScoring() can also set the identity of the Seurat object to the 
    ...
    # AAACCCAAGAAGTCAT-1      ramos        832          642   3.365385
    # AAACCCAAGAGAGTGA-1      ramos       2925         1753   4.581197
    # AAACCCAAGAGCACTG-1      ramos        976          714   3.995902
    # AAACCCAAGAGTTGAT-1      ramos        604          503   1.821192
    # AAACCCAAGATGCGAC-1      ramos       1410          980   4.751773
    #                          S.Score    G2M.Score Phase old.ident
    # AAACCCAAGAACTGAT-1 -0.0407258555 -0.057724360    G1     ramos
    # AAACCCAAGAAGTCAT-1 -0.0001567449  0.034862937   G2M     ramos
    # AAACCCAAGAGAGTGA-1 -0.0604159278 -0.094153570    G1     ramos
    # AAACCCAAGAGCACTG-1  0.0003536545  0.082905992   G2M     ramos
    # AAACCCAAGAGTTGAT-1  0.0306721353 -0.063594145     S     ramos
    ...
    # The first suggested procedure in this tutorial removes all signal associated with cell cycle.
    # In some cases, the Seurat developers
    # found that this can negatively impact downstream analysis, particularly in differentiating
    # processes (like murine hematopoiesis), where stem cells are quiescent and differentiated 
    # cells are proliferating (or vice versa). In this case, regressing out all cell cycle
    # effects can blur the distinction between stem and progenitor cells as well.
    # 
    # As an alternative, the Seurat developers suggest regressing out the difference between the G2M and S phase
    # scores. This means that signals separating non-cycling cells and cycling cells will be
    # maintained, but differences in cell cycle phase among proliferating cells (which are often
  ...
  #April 19 2023 save progress
    ...
    #may 16 2023 version
    saveRDS(pbmc.markers, "GSM6720583_markers_may_16_2023.rds")
...
############################# Cell Type Annotations    
   ...
   #PanglaoDB Augmented 2021
    ...
    ##### View the Enrichr pathway tables and see if there are redundant pathways to eliminate
