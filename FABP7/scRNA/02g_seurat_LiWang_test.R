...
project.name<-"scRNA_LiWang" 

dir.raw<-file.path("data", project.name, "GSE174554_RAW")

initDir<-function(dir.path){
  if(dir.exists(dir.path)){
    system(paste0("rm -rf ", dir.path))
  }
  system(paste0("mkdir -p ", dir.path))
}

samples.raw<-list.files(dir.raw, pattern="matrix")
samples.raw<-samples.raw[!grepl("batch2",samples.raw)]

samples<-sort(colsplit(samples.raw,"_",letters[1:6])$b)
samples<-samples[!grepl("v2",samples)]


sample.curr<-samples[1]

sample.curr<-"SF10108" # astro
#sample.curr<-"SF12704"# complex
# SF6098 segmented microglia
# SF3391 complex GBM
# SF9372 complex GBM
...
for(sample.curr in samples){
  try({
    ...
    fnames.curr<-list.files(dir.raw, pattern=paste0("_",sample.curr,"_"))
    
    initDir("data/tmp")
    for(fname in fnames.curr){
      file.copy(file.path(dir.raw,fname), file.path("data/tmp",colsplit(fname,"_",letters[1:3])$c))
    }
    ...
    write.table(df.genes, file=paste0("C:/Users/qt09n/Desktop/Technical Analyst I UHN May 4 2021/organoid group/Sofia/late prenatal human neurodevelopment resolved by single nucleus transcriptomics/output/",project.name,"/02_genes/",sample.curr,".csv"),quote=FALSE,row.names=FALSE,sep=",")
    
    saveRDS(clust.avg, file=paste0("C:/Users/qt09n/Desktop/Technical Analyst I UHN May 4 2021/organoid group/Sofia/late prenatal human neurodevelopment resolved by single nucleus transcriptomics/output/",project.name,"/02_cluster_expr/",sample.curr,".rds"))
...
#}
    ...
    saveRDS(list.enriched, file=paste0("C:/Users/qt09n/Desktop/Technical Analyst I UHN May 4 2021/organoid group/Sofia/late prenatal human neurodevelopment resolved by single nucleus transcriptomics/output/",project.name,"/02_enrichr/", sample.curr,".rds"))
    ...
    png(filename=paste0("C:/Users/qt09n/Desktop/Technical Analyst I UHN May 4 2021/organoid group/Sofia/late prenatal human neurodevelopment resolved by single nucleus transcriptomics/output/",project.name,"/02_plot/",sample.curr,".png"))
    print(p)
    dev.off()
    ...
    saveRDS(df.summary, file=paste0("C:/Users/qt09n/Desktop/Technical Analyst I UHN May 4 2021/organoid group/Sofia/late prenatal human neurodevelopment resolved by single nucleus transcriptomics/output/",project.name,"/02_cell_type/",sample.curr,".rds"))
  }) #try
}
