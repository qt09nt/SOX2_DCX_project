...
filter_gsea <- function(gsea_report_na_pos, gsea_report_for_na_neg, cell_type1, cell_type2){
  ...
  filtered_SOX2 <- SOX2_gsea[SOX2_gsea$`NOM.p.val` <= 0.25,]
  filtered_SOX2 <- filtered_SOX2[filtered_SOX2$`FDR.q.val` <= 0.25, ]
...
extract_specific_GO_db <-function(filtered_DCX, filtered_SOX2, GO_db){
  ...
  GO_merged$cell_type <- factor(GO_merged$cell_type, levels = c("SOX2", "DCX"))
